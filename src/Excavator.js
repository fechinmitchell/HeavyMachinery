...
export class Excavator {
  constructor(scene, physicsWorld) {
    ...
    this.keys = {
      w: false, s: false, a: false, d: false, // Tracks
      q: false, e: false,                   // Turret
      r: false, f: false,                   // Boom
      t: false, g: false,                   // Stick
      y: false, h: false,                   // Bucket curl
      space: false                          // Dig
    };
  ...
  createModel() {
    ...
    // Tracks (detailed with side panels)
    ...
    this.cabinMesh.rotation.x = -0.2; // Slight forward tilt

    // Counterweight (larger, curved)
    ...
    this.turretGroup.add(this.turretMesh, this.cabinMesh, this.counterweightMesh, this.boomMesh, this.stickMesh, this.bucketMesh);
    this.scene.add(this.turretGroup);
  }

  createPhysics() {
    const excavatorMaterial = new CANNON.Material({ friction: 0.8, restitution: 0.1 });

    this.baseBody = new CANNON.Body({ mass: 1000 });
    this.baseBody.addShape(new CANNON.Box(new CANNON.Vec3(0.9, 0.35, 1.9)));
    this.baseBody.position.set(0, 0.35, 0);
    this.baseBody.material = excavatorMaterial;
    this.baseBody.linearDamping = 0.95;
    this.baseBody.angularDamping = 0.95;
    this.physicsWorld.addBody(this.baseBody);

    this.turretBody = new CANNON.Body({ mass: 300 });
    this.turretBody.addShape(new CANNON.Box(new CANNON.Vec3(0.5, 0.7, 0.5)));
    ...
    this.turretConstraint = new CANNON.HingeConstraint(this.baseBody, this.turretBody, {
      ...
      pivotB: new CANNON.Vec3(0, -0.7, 0),
    ...
    this.boomBody.position.set(0, 2.5, 0);
    ...
    this.boomConstraint = new CANNON.HingeConstraint(this.turretBody, this.boomBody, {
      pivotA: new CANNON.Vec3(0, 0.7, 0),
    ...
    this.stickBody.position.set(0, 4.5, 0);
    ...
    this.bucketBody.addShape(new CANNON.Box(new CANNON.Vec3(0.35, 0.35, 0.35)));
    this.bucketBody.position.set(0, 6, 0);
  ...
  update() {
    ...
    // Tracked movement
    const velocity = this.baseBody.velocity;
    const forward = new CANNON.Vec3(0, 0, -1);
    const forwardWorld = this.baseBody.quaternion.vmult(forward);

    if (this.keys.w) {
      velocity.x = forwardWorld.x * speed;
      velocity.z = forwardWorld.z * speed;
    } else if (this.keys.s) {
      velocity.x = -forwardWorld.x * speed;
      velocity.z = -forwardWorld.z * speed;
    } else {
    ...
    if (this.keys.a) this.baseBody.angularVelocity.y = turnSpeed;
    else if (this.keys.d) this.baseBody.angularVelocity.y = -turnSpeed;
    else this.baseBody.angularVelocity.y *= damping;
